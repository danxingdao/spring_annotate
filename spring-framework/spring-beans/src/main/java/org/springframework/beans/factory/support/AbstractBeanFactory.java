...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	 ...
	 */
	/*
	 *  通过注释，可以整理出较清晰的逻辑： 检查Bean缓存，已经有缓存的Bean对象(有可能是半成品)则交给getObjectForBeanInstance方法处理；
	 *  否则先根据Bean的生命周期类型分别实例化，每种情况大致都分两步,第一步都交给createBean方法生产一个半成品的bean对象，
	 *  然后同样是将半成品的bean交给getObjectForBeanInstance方法处理。
	 */
	...
	protected <T> T doGetBean(
		...
		/*
		 * 或许很多人不理解转换对应beanName是什么意思，传入的参数name不就是beanName吗？其实不是，这里传入的参数可能是别名，也可能是FactoryBean
		...
		/* 检查缓存中或实例工厂中是否有对应的实例
		 * 为什么首先使用这段代码呢
		 * 因为在创建单例bean的时候会存在依赖注入的情况而在创建依赖的时候为了避免循环依赖
		 * spring创建bean的原则是不等bean创建完成就会将创建bean的ObjectFactory提早曝光
		 * 也就是讲ObjectFactory加入到缓存中一旦下个bean创建时候需要依赖上个bean则直接使用
		 * 
		 */
		...
		if (sharedInstance != null && args == null) {
			...
			// 分支二：没有缓存，则需要从头实例化该bean  
			// Fail if we're already creating this bean instance:
			// We're assumably within a circular reference.
			/*
			 * 只有在单例的情况下才会尝试解决循环依赖，原型模式情况下，如果存在
			 * A中有B的属性，B中有A的属性，那么当依赖注入的时候就会产生A还没有创建玩的时候因为
			 * 对于B的创建再次返回创建A，造成循环依赖，也就是下面的情况。
			 */
			/*
			 * 只有单例的属性注入才会解决循环依赖，因为单例是需要先用构造方法创建出对象来，才会进行属性注入，
			 * 因此不能解决构造方法注入的循环依赖 
			 */
			 ...
			 * parentBeanFactory如果为空，则其他一切都是浮云，这个没什么说的，但是!containsBeanDefinition(beanName)就比较重要了，
			 * 它是在检测如果当前加载的XML配置文件中不包含beanName所对应的配置，就只能到parentBeanFactory去尝试下了，然后再去递归的调用getBean方法。
			 */
	 ...
	 */
	/*
	 * 用于检测当前bean是否是FactoryBean类型的bean,如果不是，则直接返回对象，如果是，则获取FactoryBean的对象
	 */
