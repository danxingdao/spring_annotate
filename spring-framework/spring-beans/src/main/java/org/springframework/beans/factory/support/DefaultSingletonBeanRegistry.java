...
public class DefaultSingletonBeanRegistry extends SimpleAliasRegistry implements SingletonBeanRegistry {
	...
	/* 保存已调用过ObjectFactory 里已调用过 getObject方法的对象，此时createBean已被调用 */
	 ...
	 * 这里是原始bean，即使用工厂方法或构造方法创建出来的对象，一旦对象最终创建好，此引用信息将删除
	 */
	 ...
	 */
	/*
	 * 记录加载状态，也就是通过this.singletonsCurrentlyIn Creation.add(beanName)将当前正要创建的bean记录在缓存中，
	 * 这样便可以对循环依赖进行检测。
	 */
