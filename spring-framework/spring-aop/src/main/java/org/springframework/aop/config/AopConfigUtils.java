...
public abstract class AopConfigUtils {
	 ...
	 * 对于AOP的实现，基本上都是靠AnnotationAwareAspectJAutoProxyCreator去完成，它可以根据@Point注解定义的切点来自动代理相应匹配的bean。
	 * 但是为了配置方便，Spring使用了自定义配置来帮助我们自动注册AnnotationAwareAspectJAutoProxyCreator。
	 */
	...
	private static BeanDefinition registerOrEscalateApcAsRequired(Class<?> cls, BeanDefinitionRegistry registry, Object source) {
		...
		// 如果已存在了自动代理创建器且存在的自动代理创建器与现在的不一致那么需要根据优先级来判断到底需要使用哪个
		if (registry.containsBeanDefinition(AUTO_PROXY_CREATOR_BEAN_NAME)) {
			...
			if (!cls.getName().equals(apcDefinition.getBeanClassName())) {
				...
				if (currentPriority < requiredPriority) {
					// 改变bean最重要的就是改变bean对应的className的属性
